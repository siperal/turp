...
"You must define the environment variable DESTIASSOLOGIN and DESTISFLOGIN to define your login to connect to the dolibarr foundation server and/or mirrors server.\n";
	...
	print "On Linux:\n";
	print
"export DESTIASSOLOGIN='mylogin'; export DESTISFLOGIN='mylogin,project';\n";
	print "On Windows:\n";
...
"You must define the environment variable DESTIBETARC and DESTISTABLE to point to the\ndirectories where you want to save the generated packages.\n";
	...
	print "Example in .bashrc:\n";
	print
"export DESTIBETARC='/mnt/HDDATA1_LD/Mes Archives/Doli/dolibarr/lastbuild'\n";
	print
"export DESTISTABLE='/mnt/HDDATA1_LD/Mes Archives/Doli/dolibarr/stable'\n";
	sleep 2;
	exit 1;
}
if ( !-d $ENV{"DESTIBETARC"} || !-d $ENV{"DESTISTABLE"} ) {
	print
"Error: Directory of environment variable DESTIBETARC ($ENV{'DESTIBETARC'}) or DESTISTABLE ($ENV{'DESTISTABLE'}) does not exist.\n";
	print "$PROG.$Extension aborted.\n";
	sleep 2;
	exit 1;
}
...
elsif ( "$^O" =~ /cygwin/i || "$^O" =~ /win32/i || "$^O" =~ /msys/i ) {
	$OS = 'windows';
	$CR = "\r";
}
if ( !$OS ) {
...
$newbuild =~ s/beta(.?)/0.2.beta/gi
  ;    # beta (fedora)    (we want beta1, beta2, betax to be same package name)
$newbuild =~ s/rc(.?)/0.3.rc/gi
  ;    # rc (fedora)      (we want rc1, rc2, rcx to be same package name)
...
$newbuild =~ s/beta(.?)/2/gi
  ;    # beta    			(we want beta1, beta2, betax to be same package name)
$newbuild =~
  s/rc(.?)/3/gi;    # rc				(we want rc1, rc2, rcx to be same package name)
...
if ( $ENV{"DESTIBETARC"} && $BUILD =~ /[a-z]/i ) {
	$DESTI = $ENV{"DESTIBETARC"};
}    # Force output dir if env DESTIBETARC is defined
if ( $ENV{"DESTISTABLE"} && $BUILD =~ /^[0-9]+$/ ) {
	$DESTI = $ENV{"DESTISTABLE"};
}    # Force output dir if env DESTISTABLE is defined
if ( $ENV{"PUBLISHBETARC"} && $BUILD =~ /[a-z]/i ) {
	$PUBLISHBETARC = $ENV{"PUBLISHBETARC"};
}    # Force target site for publishing if env PUBLISHBETARC is defined
if ( $ENV{"PUBLISHSTABLE"} && $BUILD =~ /^[0-9]+$/ ) {
	$PUBLISHSTABLE = $ENV{"PUBLISHSTABLE"};
}    # Force target site for publishing if env PUBLISHSTABLE is defined
...
if ($target) {
	if ( $target eq "ALL" ) {
		foreach my $key (@LISTETARGET) {
			if ( $key ne 'SNAPSHOT' && $key ne 'SF' && $key ne 'ASSO' ) {
				$CHOOSEDTARGET{$key} = 1;
			}
		}
	}
	if ( $target ne "ALL" && $target ne "SF" && $target ne "ASSO" ) {
	...
	while ( !$found ) {
		...
		printf( " %2d - %-14s  (%s)\n",
			$cpt, "ALL (1..10)",
			"Need " . join( ",", values %REQUIREMENTTARGET ) );
		$cpt++;
		...
		print "Choose one target number or several separated with space (0 - "
		  . $cpt . "): ";
		$NUM_SCRIPT = <STDIN>;
		chomp($NUM_SCRIPT);
		if ( $NUM_SCRIPT !~ /^[0-9\s]+$/ ) {
	...
	if ( $NUM_SCRIPT eq "98" ) {
		...
		foreach my $key (@LISTETARGET) {
			if ( $key ne 'SNAPSHOT' && $key ne 'ASSO' && $key ne 'SF' ) {
				$CHOOSEDTARGET{$key} = 1;
			}
		}
	}
...
"\nCheck if ChangeLog is ok for version $MAJOR.$MINOR\.$TMPBUILDTOCHECKCHANGELOG\n";
		$ret =
`grep "ChangeLog for $MAJOR.$MINOR\.$TMPBUILDTOCHECKCHANGELOG" "$SOURCE/ChangeLog" 2>&1`;
		if ( !$ret ) {
			print color("yellow"),
"Error: The ChangeLogFile was not updated. Run the following command before building package for $MAJOR.$MINOR.$BUILD:\n",
			  color('reset');
		}
		else {
			print
"ChangeLog for $MAJOR.$MINOR\.$BUILD was found into '$SOURCE/ChangeLog'. But you can regenerate it with command:\n";
		}
		if (  !$BUILD
			|| $BUILD eq '0-alpha'
			|| $BUILD eq '0-beta'
			|| $BUILD eq '0-rc' )    # For a major or future version
		{
...
#print 'git log `git rev-list --boundary '.$MAJOR.'.'.$MINOR.'..origin/develop | grep ^- | cut -c2- | head -n 1`.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\|PERF\|SEC\|QUAL\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' | sed \'s/^* //g\' > /tmp/aaa';
			print 'git log `diff -u <(git rev-list --first-parent '
			  . ( $MAJOR - 1 )
			  . '.0)  <(git rev-list --first-parent develop) | sed -ne \'s/^ //p\' | head -1`.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/changelogtocopy';
		}
...
#print 'cd ~/git/dolibarr_'.$MAJOR.'.'.$MINOR.'; git log '.$MAJOR.'.'.$MINOR.'.'.($BUILD-1).'.. --no-merges --pretty=short --oneline | sed -e "s/^[0-9a-z]* //" | grep -e \'^FIX\|NEW\|PERF\|SEC\|QUAL\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\'| sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' > /tmp/aaa';
			print 'cd ~/git/dolibarr_'
			  ...
			  . $MINOR . '.'
			  . ( $BUILD - 1 )
			  . '.. | grep -v "Merge branch" | grep -v "Merge pull" | grep "^ " | sed -e "s/^[0-9a-z]* *//" | grep -e \'^FIX\|NEW\|PERF\|SEC\|QUAL\|CLOSE\' | sort -u | sed \'s/FIXED:/FIX:/g\' | sed \'s/FIXED :/FIX:/g\' | sed \'s/FIX :/FIX:/g\' | sed \'s/FIX /FIX: /g\' | sed \'s/CLOSE/NEW/g\' | sed \'s/NEW :/NEW:/g\' | sed \'s/NEW /NEW: /g\' | sed \'s/^* //g\' > /tmp/aaa';
		}
		print "\n";
		if ( !$ret ) {
			print
"\nPress F to force and continue anyway (or other key to stop)... ";
	...
	if ( $CHOOSEDTARGET{'-CHKSUM'} ) {
		...
		if ($ret) {
			print
"Some files exists in source directory and are not indexed neither excluded in .gitignore.\n";
			print $ret;
			print "Canceled.\n";
			exit;
		}
		...
		print 'Create xml check file with md5 checksum with command php '
		  ...
		  . $BUILD . "\n";
		$ret =
`php $SOURCE/dev/build/generate_filelist_xml.php release=$MAJOR.$MINOR.$BUILD`;
		my $retcode = $?;
		if ( $retcode != 0 ) {
		...
		if ( !-d "$NEWDESTI/signatures" ) {
			...
			make_path "$NEWDESTI/signatures"
			  or die "Failed to create path: $NEWDESTI/signatures";
		}
		print
"Copy \"$SOURCE/htdocs/install/filelist-$MAJOR.$MINOR.$BUILD.xml\" to $NEWDESTI/signatures/filelist-$MAJOR.$MINOR.$BUILD.xml\n";
		use File::Copy qw(copy);
		copy "$SOURCE/htdocs/install/filelist-$MAJOR.$MINOR.$BUILD.xml",
		  "$NEWDESTI/signatures/filelist-$MAJOR.$MINOR.$BUILD.xml";
	}
...
#$ret=`cp -pr "$SOURCE/dev/build/debian/apache/.htaccess" "$BUILDROOT/$PROJECT/dev/build/debian/apache/.htaccess"`;
		...
		$ret =
		  `rm -f  $BUILDROOT/$PROJECT/dev/build/doxygen/doxygen_warnings.log`;
		...
		$ret =
		  `rm -f  $BUILDROOT/$PROJECT/doc/images/dolibarr_screenshot10.png`;
		$ret =
		  `rm -f  $BUILDROOT/$PROJECT/doc/images/dolibarr_screenshot11.png`;
		$ret =
		  `rm -f  $BUILDROOT/$PROJECT/doc/images/dolibarr_screenshot12.png`;
		...
		print
"find $BUILDROOT/$PROJECT/htdocs/custom/* -type d -exec rm -fr {} \\;\n";
		$ret =
`find $BUILDROOT/$PROJECT/htdocs/custom/* -type d -exec rm -fr {} \\; >/dev/null 2>&1`
		  ;    # For custom we want to remove all subdirs but not files
		print
"find $BUILDROOT/$PROJECT/htdocs/custom/* -type l -exec rm -fr {} \\;\n";
		$ret =
`find $BUILDROOT/$PROJECT/htdocs/custom/* -type l -exec rm -fr {} \\; >/dev/null 2>&1`
		  ; # For custom we want to remove all subdirs, even symbolic links, but not files
...
# Removed known external modules to avoid any error when packaging from env where external modules are tested
		...
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/theme/common/fontawesome-5/svgs`;

		# Removed other test files
		$ret = `rm -fr $BUILDROOT/$PROJECT/htdocs/public/test`;
		$ret = `rm -fr $BUILDROOT/$PROJECT/test`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/Thumbs.db $BUILDROOT/$PROJECT/*/Thumbs.db $BUILDROOT/$PROJECT/*/*/Thumbs.db $BUILDROOT/$PROJECT/*/*/*/Thumbs.db $BUILDROOT/$PROJECT/*/*/*/*/Thumbs.db`;
		$ret =
`rm -f  $BUILDROOT/$PROJECT/.cvsignore $BUILDROOT/$PROJECT/*/.cvsignore $BUILDROOT/$PROJECT/*/*/.cvsignore $BUILDROOT/$PROJECT/*/*/*/.cvsignore $BUILDROOT/$PROJECT/*/*/*/*/.cvsignore $BUILDROOT/$PROJECT/*/*/*/*/*/.cvsignore $BUILDROOT/$PROJECT/*/*/*/*/*/*/.cvsignore`;
		$ret =
`rm -f  $BUILDROOT/$PROJECT/.gitignore $BUILDROOT/$PROJECT/*/.gitignore $BUILDROOT/$PROJECT/*/*/.gitignore $BUILDROOT/$PROJECT/*/*/*/.gitignore $BUILDROOT/$PROJECT/*/*/*/*/.gitignore $BUILDROOT/$PROJECT/*/*/*/*/*/.gitignore $BUILDROOT/$PROJECT/*/*/*/*/*/*/.gitignore`;
...
`rm -f  $BUILDROOT/$PROJECT/htdocs/includes/jquery/plugins/select2/release.sh`;
		...
		$ret =
`rm -f  $BUILDROOT/$PROJECT/htdocs/includes/mike42/escpos-php/example`;
		$ret =
		  `rm -f  $BUILDROOT/$PROJECT/htdocs/includes/mike42/escpos-php/test`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/mobiledetect/mobiledetectlib/.gitmodules`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/mobiledetect/mobiledetectlib/docs`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/nnnick/chartjs/.github`;
		$ret = `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/nnnick/chartjs/docs`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/nnnick/chartjs/samples`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/nnnick/chartjs/scripts`;
		...
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/fonts/dejavu-fonts-ttf-*`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/fonts/freefont-*`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/fonts/ae_fonts_*`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/fonts/utils`;
		$ret =
`rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/examples`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/tecnickcom/tcpdf/tools`;
		...
		$ret = `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/sabre/sabre/*/*/bin`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/sabre/sabre/*/*/*/bin`;
		$ret =
		  `rm -fr $BUILDROOT/$PROJECT/htdocs/includes/sabre/sabre/*/*/*/*/bin`;
	}
	...
	foreach my $target ( sort keys %CHOOSEDTARGET ) {
		...
		if ( $target eq 'SNAPSHOT' ) {
			...
			$cmd =
			  "cp -pr \"$BUILDROOT/$PROJECT\" \"$BUILDROOT/$FILENAMESNAPSHOT\"";
			$ret = `$cmd`;
...
"tar --exclude doli*.tgz --exclude doli*.deb --exclude doli*.exe --exclude doli*.xz --exclude doli*.zip --exclude doli*.rpm --exclude .cache --exclude .settings --exclude conf.php --exclude conf.php.mysql --exclude conf.php.old --exclude conf.php.postgres --directory \"$BUILDROOT\" --mode=go-w --group=500 --owner=500 -czvf \"$FILENAMESNAPSHOT.tgz\" $FILENAMESNAPSHOT";
			...
			# Move to final dir
			print
			  "Move $FILENAMESNAPSHOT.tgz to $NEWDESTI/$FILENAMESNAPSHOT.tgz\n";
			$ret =
			  `mv "$FILENAMESNAPSHOT.tgz" "$NEWDESTI/$FILENAMESNAPSHOT.tgz"`;
			next;
		}
		...
		if ( $target eq 'TGZ' ) {
			...
			$cmd =
			  "cp -pr \"$BUILDROOT/$PROJECT/\" \"$BUILDROOT/$FILENAMETGZ\"";
			$ret = `$cmd`;
			...
			$cmd =
"tar --exclude-vcs --exclude-from \"$BUILDROOT/$PROJECT/dev/build/tgz/tar_exclude.txt\" --directory \"$BUILDROOT\" --mode=go-w --group=500 --owner=500 -czvf \"$BUILDROOT/$FILENAMETGZ.tgz\" $FILENAMETGZ";
			print "$cmd\n";
			$ret = `$cmd`;

			# Move to final dir
			print
"Move $BUILDROOT/$FILENAMETGZ.tgz to $NEWDESTI/$FILENAMETGZ.tgz\n";
			$ret =
			  `mv "$BUILDROOT/$FILENAMETGZ.tgz" "$NEWDESTI/$FILENAMETGZ.tgz"`;
			next;
		}
		...
		if ( $target eq 'ZIP' ) {
			...
			chdir("$BUILDROOT");
			$cmd =
"7z a -r -tzip -xr\@\"$BUILDROOT\/$FILENAMEZIP\/dev\/build\/zip\/zip_exclude.txt\" -mx $BUILDROOT/$FILENAMEZIP.zip $FILENAMEZIP\/*";
			print $cmd. "\n";
			$ret = `$cmd`;
			chdir("$olddir");
       ...
       #$ret=`rm -fr $BUILDROOT/$FILENAMETGZ2/htdocs/includes/ckeditor/_source`;
			...
			$cmd =
			  "find $BUILDROOT/$FILENAMETGZ2 -type f -exec chmod 644 {} \\; ";
			$ret = `$cmd`;

			# Build tgz
			print "Compress $FILENAMETGZ2 into $FILENAMETGZ2.tgz...\n";
			$ret =
`tar --exclude-from "$SOURCE/dev/build/tgz/tar_exclude.txt" --directory "$BUILDROOT" -czvf "$BUILDROOT/$FILENAMETGZ2.tgz" $FILENAMETGZ2`;

			print
"Move $BUILDROOT/$FILENAMETGZ2.tgz to $RPMDIR/SOURCES/$FILENAMETGZ2.tgz\n";
			$cmd =
"mv $BUILDROOT/$FILENAMETGZ2.tgz $RPMDIR/SOURCES/$FILENAMETGZ2.tgz";
			$ret = `$cmd`;

			$BUILDFIC    = "${FILENAME}.spec";
			$BUILDFICSRC = "${FILENAME}_generic.spec";
			if ( $target =~ /FEDO/i ) {
				$BUILDFICSRC = "${FILENAME}_fedora.spec";
			}
			if ( $target =~ /MAND/i ) {
				$BUILDFICSRC = "${FILENAME}_mandriva.spec";
			}
			if ( $target =~ /OPEN/i ) {
				$BUILDFICSRC = "${FILENAME}_opensuse.spec";
			}
			...
			$changelogstring = "* "
			  . $datestring
			  . " Laurent Destailleur (eldy) $MAJOR.$MINOR.$REL1-$RPMSUBVERSION\n- Upstream release\n";

			print
"Generate file $BUILDROOT/$BUILDFIC from $SOURCE/dev/build/rpm/${BUILDFICSRC}\n";
			open( my $SPECFROM, "<", "$SOURCE/dev/build/rpm/${BUILDFICSRC}" )
			  or die "Error";
			...
			$ret =
`cp "$SOURCE/dev/build/rpm/dolibarr-forrpm.patch" "$RPMDIR/SOURCES"`;
			$ret = `chmod 644 $RPMDIR/SOURCES/dolibarr-forrpm.patch`;

			print
"Launch RPM build (rpmbuild --clean -ba $BUILDROOT/${BUILDFIC})\n";

			#$ret=`rpmbuild -vvvv --clean -ba $BUILDROOT/${BUILDFIC}`;
			$ret = `rpmbuild --clean -ba $BUILDROOT/${BUILDFIC}`;

			# Move to final dir
			print "Move $RPMDIR/RPMS/"
			  . $ARCH . "/"
			  . $FILENAMETGZ2 . "-"
			  . $RPMSUBVERSION . "*."
			  . $ARCH
			...
			$cmd =
			  ...
			  . ".rpm \"$NEWDESTI/\"";
			$ret = `$cmd`;
			print "Move $RPMDIR/SRPMS/"
			  . $FILENAMETGZ2 . "-"
			  . $RPMSUBVERSION
			  . "*.src.rpm into $NEWDESTI/"
			  . $FILENAMETGZ2 . "-"
			  . $RPMSUBVERSION
			  . "*.src.rpm\n";
			$cmd =
		...
		if ( $target eq 'DEB' ) {
			...
			if ( $NEWDESTI =~ /stable/ ) {
				...
				if ( -d $DESTI . '/package_debian-ubuntu' ) {
					$NEWDESTI = $DESTI . '/package_debian-ubuntu';
				}
			}
			...
			$ret = `$cmd`;
			$cmd =
"cp -pr \"$BUILDROOT/$PROJECT/dev/build/debian/apache/.htaccess\" \"$BUILDROOT/$PROJECT.tmp/dev/build/debian/apache/.htaccess\"";
			$ret = `$cmd`;
			...
			$ret =
			  `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.config`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.desktop`;
			$ret =
			  `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.docs`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.install`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.lintian-overrides`;
			$ret =
			  `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.postrm`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.postinst`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.templates`;
			$ret =
`rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/dolibarr.templates.futur`;
			$ret = `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/rules`;
			$ret =
			  `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/README.Debian`;
			$ret =
			  `rm -f  $BUILDROOT/$PROJECT.tmp/dev/build/debian/README.howto`;
...
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/ckeditor/ckeditor/plugins/wsc/LICENSE.md`;
			...
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/flot/LICENSE.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/datatables/extensions/ColReorder/License.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/datatables/extensions/ColVis/License.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/datatables/extensions/FixedColumns/License.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/datatables/extensions/Responsive/License.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/datatables/license.txt`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/jquery/plugins/select2/LICENSE`;
			$ret =
`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/mike42/escpos-php/LICENSE.md`;
			$ret =
...
#$ret=`rm -fr $BUILDROOT/$PROJECT.tmp/htdocs/includes/ckeditor/ckeditor/_source`;
			...
			$ret =
`find $BUILDROOT/$PROJECT.tmp/ -type f -name '.editorconfig' -exec rm {} \\;`;
			$ret =
`find $BUILDROOT/$PROJECT.tmp/ -type f -name '.travis.yml' -exec rm {} \\;`;

			# Rename upstream changelog to match debian rules
			$ret =
`mv $BUILDROOT/$PROJECT.tmp/ChangeLog $BUILDROOT/$PROJECT.tmp/changelog`;

			# Prepare source package (init debian dir)
			print "Create directory $BUILDROOT/$PROJECT.tmp/debian\n";
			$ret = `mkdir "$BUILDROOT/$PROJECT.tmp/debian"`;
			print
"Copy $SOURCE/dev/build/debian/xxx to $BUILDROOT/$PROJECT.tmp/debian\n";

# Add files for dpkg-source (changelog)
#$ret=`cp -f  "$SOURCE/dev/build/debian/changelog"      "$BUILDROOT/$PROJECT.tmp/debian"`;
			open( my $SPECFROM2, "<", "$SOURCE/dev/build/debian/changelog" )
			  or die "Error";
			open( my $SPECTO2, ">", "$BUILDROOT/$PROJECT.tmp/debian/changelog" )
			  or die "Error";
			while (<$SPECFROM2>) {
			...
			$ret =
`cp -f  "$SOURCE/dev/build/debian/compat"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/control"        "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/copyright"      "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.desktop"        	"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.docs"        		"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.install" 	        "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.lintian-overrides"  "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.xpm"  		      	"$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/rules"          "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/watch"          "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -fr "$SOURCE/dev/build/debian/patches"        "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -fr "$SOURCE/dev/build/debian/po"             "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -fr "$SOURCE/dev/build/debian/source"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -fr "$SOURCE/dev/build/debian/apache"         "$BUILDROOT/$PROJECT.tmp/debian/apache"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/apache/.htaccess" "$BUILDROOT/$PROJECT.tmp/debian/apache"`;
			$ret =
`cp -fr "$SOURCE/dev/build/debian/lighttpd"       "$BUILDROOT/$PROJECT.tmp/debian/lighttpd"`;

			# Add files also required to build binary package
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.config"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.postinst"       "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.postrm"         "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/dolibarr.templates"      "$BUILDROOT/$PROJECT.tmp/debian"`;
			$ret =
`cp -f  "$SOURCE/dev/build/debian/install.forced.php.install"      "$BUILDROOT/$PROJECT.tmp/debian"`;
...
"tar --exclude-vcs --exclude-from \"$BUILDROOT/$PROJECT/dev/build/tgz/tar_exclude.txt\" --directory \"$BUILDROOT\" --mode=go-w --group=500 --owner=500 -czvf \"$BUILDROOT/$FILENAMEDEBNATIVE.orig.tar.gz\" $PROJECT-$MAJOR.$MINOR.$build";
			...
			print
			  "Go into directory $BUILDROOT/$PROJECT-$MAJOR.$MINOR.$build\n";
...
#$ret=`mv $BUILDROOT/*.debian.tar.xz "$NEWDESTI/"`;		# xz file is generated when build/debian/sources/option
		...
		if ( $target eq 'EXEDOLIWAMP' ) {
			...
			if ( $NEWDESTI =~ /stable/ ) {
				...
				if ( -d $DESTI . '/package_windows' ) {
					$NEWDESTI = $DESTI . '/package_windows';
				}
			}
...
#$ret=`cat "$SOURCE/dev/build/exe/doliwamp/doliwamp.iss" | sed -e 's/__FILENAMEEXEDOLIWAMP__/$FILENAMEEXEDOLIWAMP/g' > "$SOURCE/build/exe/doliwamp/doliwamp.tmp.iss"`;
			open( my $IN3, '<',
			  ...
			  or die $!;
			open( my $OUT, '>',
				"$SOURCE/dev/build/exe/doliwamp/doliwamp.tmp.iss" )
			  or die $!;
			while (<$IN3>) {
...
'Dolibarr installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
			...
			"$DESTI/package_debian-ubuntu/${FILENAMEDEB}_amd64.changes" =>
			  'none',    # none means it won't be published on SF
			...
			"$DESTI/package_debian-ubuntu/${FILENAMEDEBSHORT}.orig.tar.gz" =>
			  'none',    # none means it won't be published on SF
			"$DESTI/package_windows/$FILENAMEEXEDOLIWAMP.exe" =>
...
'Dolibarr installer for Fedora-Redhat-Mandriva-Opensuse (DoliRpm)',
		...
		foreach my $file ( sort keys %filestoscansf ) {
			...
			print( $filedate
				? " - " . strftime( "%Y-%m-%d %H:%M:%S", localtime($filedate) )
				: ""
			);
			print "\n";
		}
		...
		if ( $target eq 'SF' || $target eq 'ASSO' ) {
			...
			if ( $target eq 'ASSO' && $BUILD =~ /[a-z]/i ) {
				$PUBLISH = $PUBLISHBETARC . '/lastbuild';
			}
			if ( $target eq 'ASSO' && $BUILD =~ /^[0-9]+$/ ) {
				$PUBLISH = $PUBLISHBETARC . '/stable';
			}
