...
if ($dirins && $action == 'initmodule' && $modulename /* && $user->hasRight("modulebuilder", "run") // already checked */) {
...
// Init an object
if ($dirins && $action == 'initobject' && $module && $objectname /* && $user->hasRight("modulebuilder", "run") // already checked */) {
	$warning = 0;
					...
					try {
						...
						if (!empty($tmpobject)) {
							...
							if (!empty($properties)) {
								...
								print '<tr><td><span class="warning">'.$langs->trans('Property array ->fields not found in the class. The class was probably not generated by modulebuilder').'</warning></td></tr>';
							}
							print '</table>';
							print '</div>';
