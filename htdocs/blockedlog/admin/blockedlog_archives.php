...
if ($action == 'check' || $action == 'checkconfirmed') {
	...
	if (preg_match('/^'.$regnumber.'/', $secretkey) && !getDolGlobalString("BLOCKEDLOG_FOR_TAX_AUDITOR")) {
		...
		if (!getDolGlobalString("BLOCKEDLOG_FOR_TAX_AUDITOR")) {
			...
			// TODO Search the full registration number and prefill value
			$inputregistrationnumber = '123';
